<template>
    <div :class="wrapClasses">
        <template v-if="type !== 'textarea'">
            ...
            <i class="ivu-icon" :style="clearInputStyle" :class="['ivu-icon-ios-close-circle', prefixCls + '-icon', prefixCls + '-icon-clear' , prefixCls + '-icon-normal']" v-if="clearable && currentValue && !itemDisabled" @click="handleClear"></i>
            ...
            <input
                ...
                @input="handleInput"
                @change="handleChange">
            <div ref="eleAppend" :class="[prefixCls + '-group-append']" v-if="append" v-show="slotReady"><slot name="append"></slot></div>
            <div ref="eleSearch" :class="[prefixCls + '-group-append', prefixCls + '-search']" v-else-if="search && enterButton" @click="handleSearch">
...
<script>
    ...
    export default {
        ...
        mounted () {
            ...
            this.resizeTextarea();
            vm.$nextTick(() => {
              if (vm.$slots&&vm.$slots.append&&vm.$slots.append.length>0) {
                  for(var slot of vm.$slots.append) {
                      if(slot.elm && slot.elm.offsetWidth) vm.clearInputStyle = {right:(slot.elm.offsetWidth+10)+"px"};
                  }
              }
              if(vm.$refs.eleSearch){
                  vm.clearInputStyle = {right:(vm.$refs.eleSearch.offsetWidth+0)+"px"};
              }
            })
        }
    };
