        ...
        <ul :class="classes">
            <li @contextmenu.stop="handleContextmenu(data, $event)" @selectstart.stop="handlePreventSelect(data, $event)">
                ...
                <span :class="titleClasses" @click="handleSelect">
                    ...
                    <template v-else>{{ data.title || data[label] }}</template>
                </span>
                <Tree-node
                        v-if="data.expand"
    ...
    export default {
        ...
        methods: {
            handleExpand () {
                ...
                // Vue.js 2.6.9 对 transition 的 appear 进行了调整，导致 bcvvui 初始化时无动画，加此方法来判断通过点击箭头展开时，加 appear，否则初始渲染时 appear 为 false
